...
PB_Global_StateDefiner=function(app){
		...
		me.CreateWrappedState=function(templateUrlOrRawState,params,parent,isHtm,ownerOfView){
			...
			if(angular.isString(templateUrlOrRawState)){
				...
				if(!isHtm){
					templateUrlFn=function(){
					    return ProjectBaseGlobalVar.UrlMappingPrefix + templateUrl;//no qs to ensure get view from server only once and use cache thereafter
					};
					...
					ResultGetter=['pb', 'customizedResultGetter','$stateParams',function(pb, customizedResultGetter,$stateParams) {
						...
						var url=ProjectBaseGlobalVar.UrlMappingPrefix+uri;
						if(!url) return null;
			...
			return {
				...
				WithChild:function(childname,childparams){
					...
					var ishtm=childname.endWith('.htm') || childname.endWith('.html');
				...
				ForceViewByAction:function(url){
					...
					this.state.templateUrl=ProjectBaseGlobalVar.UrlMappingPrefix + url+"?"+ProjectBaseGlobalVar.KeyForViewModelOnly+"=false";
				    this.state.templateFn=null;
					return this;
				},
				BranchView:function(name,params){
					if(!this.isForView)throw "meant to be called by a wrapped view";
					var isHtm=name.endWith('.htm') || name.endWith('.html');
		...
		me.MultiViewsState=function(name,params,parent){
			...
			if(!parent)parent=me.DefaultLayout;
			var ws=me.CreateWrappedState(name,params,parent,isHtm);
			ws.state.name="MultiViewsLayout_"+ws.state.name;
			me.tmpStates[ws.state.name]=ws.state;
		    ws.state['abstract'] = true;
			...
			return {
				...
				BranchView:function(name,params){
					var isHtm=name.endWith('.htm') || name.endWith('.html');
					var wrappedview=me.CreateWrappedState(name,params,undefined,isHtm,this.state.name);
...
function pbFn(validationManager,$http,$state,$translate,$parse,$window,$injector,$timeout,$q,pbui){
	var $=function(ele){
	...
	var AjaxSubmitMain=function (control, qsJson, attrObj, funcExecuteResult, argsForFunc,overrideSuper) {	
	    var form,url,processingAreaId,bindTargetId,confirmText,method,ajaxData,ajaxBodyData;
    ...
    var ValidateOnSubmit = function(formElement) {
...
pbFn.$inject = ['validationManager','$http','$state','$translate','$parse','$window','$injector','$timeout','$q','pbui'];
...
pbm.directive("ajaxUrl", ['pb',function factory(pb) {
	    var directiveDefinitionObject = {
	      priority: 0,
	      restrict: 'A',
