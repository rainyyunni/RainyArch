...
public class ModelMetadataProvider implements TypeFilter { 
	...
	private static ModelMetadataProvider current;
	...
	public static ModelMetadataProvider getCurrent() {
		return current;
	}
	public static ModelMetadataProvider setCurrent(ModelMetadataProvider value) {
	...
	public void CreateMetaDatas(){
		...
		for(Class<?> clazz:keys){
			if(metadatas.get(clazz)==null){
				CreateMetadata(null,clazz,null,null);
			}
		}
	}
	protected ModelMetadata CreateMetadata(Class<?> containerType, Class<?> modelType, String propertyName,Method propertyGetter) {
        ...
        if(containerType==null){//the class itself
        	...
        	String displayName=DisplayName(null,modelType,null,null);
        	...
        	for(Class<?> innerClass : modelType.getDeclaredClasses()){
    			if(metadatas.get(innerClass)==null){
    				CreateMetadata(null,innerClass,null,null);
    			}
            }
        	...
        	for(PropertyDescriptor pd : pds){
            	...
            	meta.getProperties().put(prop, CreateMetadata(modelType,getter.getReturnType(),prop,getter));
            }
            return meta;
        }else{//is a property
        	if(!LeafModelTypes.contains(modelType) && !modelType.isArray() && !java.util.Collection.class.isAssignableFrom(modelType)  && !DictEnum.class.isAssignableFrom(modelType)){
        		...
        		if(propertytypemeta==null){
        			propertytypemeta=CreateMetadata(null,modelType,null,null);
        		}
        	...
        	meta.setModelType(modelType);
        	String displayName=DisplayName(containerType,modelType,propertyName,propertyGetter);
        	meta.setDisplayName(displayName);
        	...
        	if(returnType==String.class ){
        		Size size=propertyGetter.getAnnotation(Size.class);
        		if(size!=null)meta.setMaxLength(size.max());
        	}
        	if(primitiveNotNullTypes.contains(returnType) || propertyGetter.getAnnotation(NotNull.class)!=null){
...
//    				);
    ...
    protected String DisplayName(Class<?> containerType, Class<?> modelType, String propertyName,Method propertyGetter)
    {
    	...
    	nameAttr=propertyGetter.getAnnotation(DisplayName.class);
    	if(nameAttr!=null) return nameAttr.value();

        String displayName= DisplayNameForProp(containerType, propertyName,propertyGetter);
        ...
        if (StringUtils.isEmpty(displayName) && propertyName != null && propertyName.endsWith(DisplayExtension.getSurfixForDisplay()) && containerType != null)
        {
        	...
        	displayName = DisplayNameForProp(containerType, propertyName.replace(DisplayExtension.getSurfixForDisplay(), ""),soureMethod);
        }
    ...
    private String DisplayNameForProp(Class<?> containerType ,String propertyName,Method propertyGetter)
    {
        String displayname = null;
        String resourceName = "";
        DisplayNameKey keyattr =propertyGetter.getAnnotation(DisplayNameKey.class);
        if(keyattr==null)
        {
        ...
        if (keyattr != null)
        {
            ...
            displayname= Res.DisplayName(resourceName);
            if (!StringUtils.isEmpty(displayname)) return displayname;
        }
        DisplayName nameAttr=null;
        if (containerType != null)
        {
        ...
        displayname =Res.DisplayName(resourceName);
        if ((StringUtils.isEmpty(displayname)||displayname==resourceName) && pname!=null)
            displayname =Res.DisplayName(pname);
        return displayname;
    }
}
