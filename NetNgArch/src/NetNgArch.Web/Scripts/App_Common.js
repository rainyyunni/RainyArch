...
def.SetAjaxNavRootState('/Home/MainFrameLoggedIn');
...
app.constant('App_UniqueCheckerUri', '/Common/CheckUnique?name=');

//menu text should be unique
app.constant('App_MenuData', [
               { text: 'Sample', funcCode: 'M_Sample', stateRef: '', subMenus: [
                                                        { text: 'Task', funcCode: 'M_Task', stateName: '/TA/Task/List', stateParam: 'UrlTranslate:1' },
                                                        { text: 'TaskItem', funcCode: 'M_TaskItem', stateName: '/TA/TaskItem/List' },
                                                        { text: 'MemberNote', funcCode: 'M_MemberNote', stateName: '/CZ/MemberNote/List'}]
               },
               { text: 'System', funcCode: 'M_System', subMenus: [
                                                            { text: 'Corp', funcCode: 'M_Corp', stateName: '/GN/Corp/Edit' },
                                                            { text: 'Dept', funcCode: 'M_Dept', stateName: '/GN/Dept/List' },
...
//----------------------config components-------------------------------------------------------------------------------
app.config(['$stateProvider', '$urlRouterProvider', '$httpProvider', '$translateProvider', 'pbProvider', 'pbuiProvider','App_DefaultState',
            function ($stateProvider, $urlRouterProvider, $httpProvider, $translateProvider, pbProvider,pbuiProvider, App_DefaultState) {
                ...
                pbuiProvider.SetTplPath('../Scripts/tpl');
            } ]);
app.run(['validator', 'myCustomErrorMessageResolver', 'myCustomElementModifier', 'defaultErrorMessageResolver', 'App_FuncTree',
			function (validator, myCustomErrorMessageResolver, myCustomElementModifier, defaultErrorMessageResolver, App_FuncTree) {
        ...
        var resolve = function (errorType, el) {
            ...
            if (el.attr('pb-valmsg')) {
                try {
                    ...
                    $log.error("pb-valmsg's value is invalid, should be a json string:" + el.attr('-'));
                }
            }
