...
def.ContentState('Edit', "id").Controller(['pb', 'serverVm', '$scope',function (pb, serverVm, $scope) {
    ...
    c.ExecuteTrueResult = function (elementid) {
        pb.AjaxNavBack();
    };
...
def.ns="";
def.RootState('/home/Home/MainFrameLoggedIn.htm').NoResolve().NoController();
        ...
        var directiveDefinitionObject = {
            ...
            controller: ['$scope', '$element', '$attrs', '$transclude',function ($scope, $element, $attrs, $transclude) {
                ...
                d.AjaxNavRefresh = function () {
                	var state=pb.AjaxNavData.Stack[pb.AjaxNavData.Stack.length-1].State;
                	var stateParams=pb.AjaxNavData.Stack[pb.AjaxNavData.Stack.length-1].StateParams;
                ...
                d.AjaxNavTo = function (state,stateParams) {
                    while(pb.AjaxNavData.Stack.length>0 && state!=pb.AjaxNavData.Stack[pb.AjaxNavData.Stack.length-1].State){
                    	pb.AjaxNavData.Stack.pop();
                    }
                    $state.go(state,stateParams,{inherit:false,reload:true});
                };
            }]
